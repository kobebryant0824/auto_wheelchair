...
def main():
    ...
    match_red = os.path.join(args.data_dir, 'red', '*.jpg')
    paths_red = glob.glob(match_red)

    match_green = os.path.join(args.data_dir, 'green', '*.jpg')
    paths_green = glob.glob(match_green)

    match_back = os.path.join(args.data_dir, 'back', '*.jpg')
    paths_back = glob.glob(match_back)
    ...
    n_train = len(paths_left) + len(paths_right) + len(paths_stop) + len(paths_other) + len(paths_red) + len(paths_green) + len(paths_back)
    ...
    n_red = len(paths_red)
    n_green = len(paths_green)
    n_back = len(paths_back)
    ...
    end_ind = n_left + n_right + n_stop + n_red
    label[begin_ind:end_ind, 3] = 1.0

    begin_ind = n_left + n_right + n_stop + n_red
    end_ind = n_left + n_right + n_stop + n_red + n_green
    label[begin_ind:end_ind, 4] = 1.0

    begin_ind = n_right + n_left + n_stop + n_red + n_green
    end_ind = n_left + n_red + n_right + n_green + n_stop + n_back
    label[begin_ind:end_ind, 5] = 1.0

    begin_ind = n_right + n_left + n_stop + n_red + n_green + n_back
    end_ind = n_left + n_right + n_stop + n_red + n_green + n_back + n_other
    label[begin_ind:end_ind, 6] = 1.0
